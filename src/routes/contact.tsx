  ...
  return (
    <main style={{ maxWidth: 560, margin: '0 auto', padding: '4rem 1rem' }}>
      <div style={{ textAlign: 'center', marginBottom: 48 }} className="animate-fade-up">
        ...
        <p style={{ color: 'var(--text-muted)', marginTop: 12, fontSize: '1.05rem' }}>We usually reply within a few hours.</p>
      </div>
      {sent ? (
        <div className="reveal" style={{ background: 'var(--accent-subtle)', border: '1px solid var(--accent-border)', borderRadius: 24, padding: '48px 32px', textAlign: 'center', boxShadow: 'var(--shadow-glow)' }}>
          ...
          <p style={{ color: 'var(--text-muted)' }}>Thanks for reaching out. Our team will get back to you shortly.</p>
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="reveal" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 24, padding: 32, display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div>
            <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 600, color: 'var(--text-muted)', marginBottom: 8 }}>Email Address</label>
            <input type="email" required placeholder="your@email.com" className="pg-input" />
          </div>
          <div>
          ...
          <button type="submit" className="pg-btn-glow" style={{ width: '100%', fontSize: '1rem', padding: '14px' }}>Send Message</button>
        </form>
      )}
    </main>
  )
