...
export function Logo({ height = 'h-9' }: { height?: string }) {
  ...
  useEffect(() => {
    ...
    setSiteName(localStorage.getItem('gm_site_name') || 'NexusGear')
    function onLogo() { setLogoImage(getLogoImage()) }
    function onBranding() { setSiteName(localStorage.getItem('gm_site_name') || 'NexusGear') }
    window.addEventListener('pg_logo_updated', onLogo)
    window.addEventListener('gm_branding_updated', onBranding)
    return () => {
      window.removeEventListener('pg_logo_updated', onLogo)
      window.removeEventListener('gm_branding_updated', onBranding)
    }
  }, [])

  if (logoImage) return <img src={logoImage} alt={siteName} className={`${height} w-auto object-contain`} />
  ...
  return (
    <span style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '1.5rem', letterSpacing: '-0.02em' }}>
      <span className="gradient-text">{siteName.slice(0, half)}</span>
      <span style={{ color: 'var(--text)' }}>{siteName.slice(half)}</span>
    </span>
  )
}
  ...
  return (
    <button onClick={toggle} title="Toggle theme" style={{
      ...
      <span style={{
        ...
        transition: 'transform 0.3s', transform: theme === 'dark' ? 'translateX(0)' : 'translateX(20px)',
...
export function AuthModal({ mode, onClose }: { mode: 'login' | 'register'; onClose: () => void }) {
  ...
  async function handleSubmit() {
    ...
    const body = tab === 'register' ? { email, password, data: { full_name: name } } : { email, password }
    try {
  ...
  return (
    <div className="auth-modal" onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div className="auth-modal-box">
        ...
        {tab === 'register' && <input className="pg-input" placeholder="Full name" value={name} onChange={e => setName(e.target.value)} style={{ marginBottom: 12 }} />}
...
function Header() {
  ...
  function logout() {
    localStorage.removeItem('pg_user'); localStorage.removeItem('pg_token')
    setUser(null); window.dispatchEvent(new CustomEvent('pg_auth_updated'))
  }
      ...
      <header className="pg-nav" style={{ position: 'sticky', top: 0, zIndex: 50 }}>
            ...
            {user ? (
              ...
              <div style={{ display: 'flex', gap: 8, marginLeft: 4 }}>
                <button onClick={() => setAuthModal('login')} style={{ background: 'transparent', border: '1px solid var(--border)', color: 'var(--text)', borderRadius: 10, padding: '7px 16px', fontSize: '0.875rem', cursor: 'pointer', fontFamily: 'var(--font-body)', fontWeight: 600, transition: 'all 0.2s' }}
                  ...
                  onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.color = 'var(--text)' }}>Login</button>
                <button onClick={() => setAuthModal('register')} className="pg-btn-glow" style={{ padding: '7px 16px', fontSize: '0.875rem' }}>Register</button>
              </div>
            )}
            <ThemeToggle />
          </nav>
        ...
        {menuOpen && (
          <div style={{ borderTop: '1px solid var(--border)', background: 'var(--nav-bg)', padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 4 }}>
            {[['/', 'Shop'], ['/about', 'About'], ['/contact', 'Contact']].map(([to, label]) => (
              <Link key={to} to={to} onClick={() => setMenuOpen(false)} style={{ color: 'var(--text-muted)', textDecoration: 'none', fontSize: '0.9rem', fontWeight: 500, padding: '10px 12px', borderRadius: 8 }}>{label}</Link>
            ))}
        ...
        <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', justifyContent: 'center' }}>
          {[['/', 'Shop'], ['/about', 'About'], ['/contact', 'Contact']].map(([to, label]) => (
            <Link key={to} to={to} style={{ color: 'var(--text-muted)', textDecoration: 'none', fontSize: '0.875rem', transition: 'color 0.2s' }}
              ...
              onMouseLeave={e => { (e.target as HTMLElement).style.color = 'var(--text-muted)' }}>{label}</Link>
          ))}
    
        </div>
        <p style={{ color: 'var(--text-faint)', fontSize: '0.8rem', margin: 0 }}>© {new Date().getFullYear()} {siteName}. All rights reserved.</p>
      </div>
    </footer>
  )
}
